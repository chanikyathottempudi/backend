...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'username', 'email', 'first_name', 'last_name', 'password', 'is_staff', 'is_active', 'date_joined', 
                  'employee_id', 'role', 'profile_picture', 'phone_number', 'permissions_summary', 'full_name', 'license_number',
    ...
    def create(self, validated_data):
        ...
        employee_id = validated_data.pop('employee_id', license_number) # Use license_number if employee_id not provided
        ...
        if not employee_id:
             raise serializers.ValidationError({"employee_id": "Employee ID or License Number is required."})

        # We'll use employee_id as the username if one wasn't provided
        if 'username' not in validated_data:
            validated_data['username'] = employee_id
            
        # Use create_user to ensure password hashing
        if not password:
            password = employee_id # Default password is the License/Employee ID
